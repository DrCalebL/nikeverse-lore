...
**⛔ RULED 2026-07-29 — no longer open.** Owner: *"follow my current game's definitions — corrupted nikes are reversible, retnuhxed is too far gone."* The taxonomy STANDS; the threshold between the terms is reversibility. Settled decisively by the shipped DB (26 Retnuhxed, all `is_nike=0`, never catchable; anchor Corrupted Nikes `is_nike=1`, purifiable and bondable). The turned cohort were **Corrupted**, not Retnuhxed — which is why they came back. See `claims.json` → `cb1-retnuhxed-as-state` and `CANON.md` §3.1. **The original finding is kept below as the record of the over-read.** Formerly: **RULING — `status: open`, put to the owner; do not edit.** Most of this lens's claims were refuted (the reversal is art direction, the mechanism is inferred from an off-page gap, and `CANON.md` §2.1's husk ruling is expressly bounded). What **survives** is narrow and real: a living, uncorrupted Nike was turned and turned back, which is hard to reconcile with *"a separate kind of being altogether."* Note the lore already carries the comic's version and does not know it — `timeline/prime-era.json:126–127`: *"Temporary corruption / Some soldiers and even Stoner Nike are temporarily corrupted during the battle."* Ask which thing the word names before anyone edits. See Q2.
...
**Q2 — What does the word "Retnuhxed" name?** ⛔ **ANSWERED 2026-07-29 — NEITHER (a) NOR (b).** It names ONE thing: the irreversible kind. The reversible state is **Corrupted**. The recommendation of (a) below — that Retnuhxed also names a reversible state — is **the reading the owner struck**; do not adopt it. Kept as the record.
